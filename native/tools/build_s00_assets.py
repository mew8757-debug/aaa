...
def compile_native_action_tree(node):
    ...
    if node["children"]:
        ...
        # 0x6E probability test. The original command stores a
        # single percentage value. S23 nests 40%, then 50% in the
        # preceding else branch, yielding the original three-way random path.
        if (
            cid == 0x6E
            and params
            and 0 <= int(params[0]) <= 100
        ):
            return (
                {
...
def extract_r29_departure_variants(blob):
    ...
    section = next(
        (
            row for row in departure["sections"]
            if row["section"] == 2
        ),
        None,
    )
    ...
    def collect_mode1_roster(node):
        ids = []
        stack = list(node["children"])
        while stack:
            child = stack.pop()
            if (
                child["commandId"] == 0x06
                and len(child["params"]) >= 3
                and int(child["params"][0]) == 1
            ):
                for value in child["params"][2:]:
                    if (
                        isinstance(value, int)
                        and 0 <= value < 1024
    ...
    while stack:
        node = stack.pop()
        if node["commandId"] == 0x05 and len(node["params"]) >= 2:
            first = (
                [int(v) for v in node["params"][0]]
                if isinstance(node["params"][0], list)
                else []
            )
            second = (
                [int(v) for v in node["params"][1]]
                if isinstance(node["params"][1], list)
                else []
            )
            if 1051 in first or 1051 in second:
                roster = [0] + collect_mode1_roster(node)
                dedup = []
                for cid in roster:
                    if cid not in dedup:
                        dedup.append(cid)
                variants.append({
                    "requireTrueVariables": [1051] if 1051 in first else [],
                    "requireFalseVariables": [1051] if 1051 in second else [],
                    "characterIds": dedup,
                })
        stack.extend(node["children"])

    variants.sort(
        key=lambda row: 0 if row["requireTrueVariables"] else 1
    )
    return variants
...
def extract_r16_departure_players(blob):
    # R16 uses the same five-slot deployment pattern as R15:
    # Liu Bei is mandatory, fixed members come from 0x06, and
    # selectable members are exposed as 0x2D sections in Scene 2.
    fixed = [0]
    selectable = []
    seen = {0}
...
def main(argv):
    ...
    with zipfile.ZipFile(game1_path) as game1, zipfile.ZipFile(game2_path) as game2:
        ...
        next_r_after25 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "R" and row["number"] > 25
            ),
            None,
        )
        next_s_after25 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "S" and row["number"] > 25
            ),
            None,
        )
        next_r_after25_blob = (
        ...
        next_s_after25_blob = (
            read_member_by_basename(
                game1,
                next_s_after25["filename"],
            )
            if next_s_after25
            else None
        )
        next_s_after25_map_name = (
            f"m{next_s_after25['number']:03d}.jpg"
            if next_s_after25
            else None
        )
        next_s_after25_map_bytes = (
            ...
            if next_s_after25_map_name
            else None
        )
        if (
            next_s_after25_map_bytes is None
            and next_s_after25_map_name
        ):
            next_s_after25_map_bytes = read_member_by_basename(
                game1,
                next_s_after25_map_name,
            )
        ...
        next_r_after29 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "R" and row["number"] > 29
            ),
            None,
        )
        next_s_after29 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "S" and row["number"] > 29
            ),
            None,
        )
        next_r_after29_blob = (
        ...
        next_s_after29_blob = (
            read_member_by_basename(
                game1,
                next_s_after29["filename"],
            )
            if next_s_after29
            else None
        )
        next_s_after29_map_name = (
            f"m{next_s_after29['number']:03d}.jpg"
            if next_s_after29
            else None
        )
        next_s_after29_map_bytes = (
            read_member_by_basename(
                game2,
                next_s_after29_map_name,
            )
            if next_s_after29_map_name
            else None
        )
        if (
            next_s_after29_map_bytes is None
            and next_s_after29_map_name
        ):
            next_s_after29_map_bytes = read_member_by_basename(
                game1,
                next_s_after29_map_name,
            )
        ...
        next_r_after30 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "R" and row["number"] > 30
            ),
            None,
        )
        next_s_after30 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "S" and row["number"] > 30
            ),
            None,
        )
        next_r_after30_blob = (
        ...
        next_s_after30_blob = (
            read_member_by_basename(
                game1,
                next_s_after30["filename"],
            )
            if next_s_after30
            else None
        )
        next_s_after30_map_name = (
            f"m{next_s_after30['number']:03d}.jpg"
            if next_s_after30
            else None
        )
        next_s_after30_map_bytes = (
            read_member_by_basename(
                game2,
                next_s_after30_map_name,
            )
            if next_s_after30_map_name
            else None
        )
        if (
            next_s_after30_map_bytes is None
            and next_s_after30_map_name
        ):
            next_s_after30_map_bytes = read_member_by_basename(
                game1,
                next_s_after30_map_name,
            )
        ...
        next_r_after31 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "R" and row["number"] > 31
            ),
            None,
        )
        next_s_after31 = next(
            (
                row for row in rs_inventory
                if row["kind"] == "S" and row["number"] > 31
            ),
            None,
        )
        next_r_after31_blob = (
            read_member_by_basename(game1, next_r_after31["filename"])
            if next_r_after31
            else None
        )
        next_s_after31_blob = (
            read_member_by_basename(game1, next_s_after31["filename"])
            if next_s_after31
            else None
        )
        next_s_after31_map_name = (
            f"m{next_s_after31['number']:03d}.jpg"
            if next_s_after31
            else None
        )
        next_s_after31_map_bytes = (
            read_member_by_basename(game2, next_s_after31_map_name)
            if next_s_after31_map_name
            else None
        )
        if next_s_after31_map_bytes is None and next_s_after31_map_name:
            next_s_after31_map_bytes = read_member_by_basename(
                game1,
                next_s_after31_map_name,
            )
    ...
    map19_probe = {
        "filename": "m019.jpg",
        "found": map19_bytes is not None,
        "hexzmapEntry": 19,
    }
    if map19_bytes is not None:
        try:
            map19_width, map19_height = jpeg_dimensions(map19_bytes)
            if map19_width % 48 != 0 or map19_height % 48 != 0:
                raise ValueError(
                    f"m019 dimensions not divisible by 48: "
                    f"{map19_width}x{map19_height}"
                )
            map19_cols = map19_width // 48
            map19_rows = map19_height // 48
            terrain19_cells = extract_hexzmap_cells(
                hexz,
                19,
                map19_cols,
                map19_rows,
            )
            (map_dir / "m019.jpg").write_bytes(map19_bytes)
            (battle_dir / "terrain19.bin").write_bytes(terrain19_cells)
            map19_probe.update({
                "valid": True,
                "width": map19_width,
                "height": map19_height,
                "cols": map19_cols,
                "rows": map19_rows,
                "terrainCellCount": len(terrain19_cells),
                "terrainIds": sorted(set(terrain19_cells)),
            })
        except Exception as exc:
            map19_probe.update({
                "valid": False,
    ...
    map21_probe = {
        "filename": "m021.jpg",
        "found": map21_bytes is not None,
        "hexzmapEntry": 21,
    }
    if map21_bytes is not None:
        try:
            map21_width, map21_height = jpeg_dimensions(map21_bytes)
            if map21_width % 48 != 0 or map21_height % 48 != 0:
                raise ValueError(
                    f"m021 dimensions not divisible by 48: "
                    f"{map21_width}x{map21_height}"
                )
            map21_cols = map21_width // 48
            map21_rows = map21_height // 48
            terrain21_cells = extract_hexzmap_cells(
                hexz,
                21,
                map21_cols,
                map21_rows,
            ...
            map21_probe.update({
                ...
                "cols": map21_cols,
                "rows": map21_rows,
                "terrainCellCount": len(terrain21_cells),
                "terrainIds": sorted(set(terrain21_cells)),
            })
        except Exception as exc:
            map21_probe.update({
                "valid": False,
    ...
    map22_probe = {
        "filename": "m022.jpg",
        "found": map22_bytes is not None,
        "hexzmapEntry": 22,
    }
    if map22_bytes is not None:
        try:
            map22_width, map22_height = jpeg_dimensions(map22_bytes)
            if map22_width % 48 != 0 or map22_height % 48 != 0:
                raise ValueError(
                    f"m022 dimensions not divisible by 48: "
                    f"{map22_width}x{map22_height}"
                )
            map22_cols = map22_width // 48
            map22_rows = map22_height // 48
            terrain22_cells = extract_hexzmap_cells(
                hexz,
                22,
                map22_cols,
                map22_rows,
            )
            (map_dir / "m022.jpg").write_bytes(map22_bytes)
            (battle_dir / "terrain22.bin").write_bytes(terrain22_cells)
            map22_probe.update({
                "valid": True,
                "width": map22_width,
                "height": map22_height,
                "cols": map22_cols,
                "rows": map22_rows,
                "terrainCellCount": len(terrain22_cells),
                "terrainIds": sorted(set(terrain22_cells)),
            })
        except Exception as exc:
            map22_probe.update({
                "valid": False,
    ...
    post31_map_probe = {
        ...
        "hexzmapEntry": (
            next_s_after31["number"] if next_s_after31 else None
        ),
    }

    post30_map_probe = {
        "filename": next_s_after30_map_name,
        "found": next_s_after30_map_bytes is not None,
        "hexzmapEntry": (
            next_s_after30["number"]
            if next_s_after30
            else None
        ),
    }
    ...
    post29_map_probe = {
        "filename": next_s_after29_map_name,
        "found": next_s_after29_map_bytes is not None,
        "hexzmapEntry": (
            next_s_after29["number"]
            if next_s_after29
            else None
        ),
    }
    ...
    if (
        next_s_after25
        and next_s_after25_map_bytes is not None
    ):
        try:
            post25_map_width, post25_map_height = jpeg_dimensions(
                next_s_after25_map_bytes
            )
            if (
                post25_map_width % 48 != 0
                or post25_map_height % 48 != 0
            ):
                raise ValueError(
                    "post-S25 map dimensions not divisible by 48: "
                    f"{post25_map_width}x{post25_map_height}"
                )
            post25_map_cols = post25_map_width // 48
            post25_map_rows = post25_map_height // 48
            post25_terrain_cells = extract_hexzmap_cells(
                hexz,
                next_s_after25["number"],
                post25_map_cols,
                post25_map_rows,
            ...
            post25_map_probe.update({
                ...
                "height": post25_map_height,
                "cols": post25_map_cols,
                "rows": post25_map_rows,
                "terrainCellCount": len(post25_terrain_cells),
                "terrainIds": sorted(set(post25_terrain_cells)),
            })
        except Exception as exc:
            post25_map_probe.update({
                "valid": False,
    ...
    if (
        next_s_after29
        and next_s_after29_map_bytes is not None
    ):
        try:
            post29_map_width, post29_map_height = jpeg_dimensions(
                next_s_after29_map_bytes
            )
            if (
                post29_map_width % 48 != 0
                or post29_map_height % 48 != 0
            ):
                raise ValueError(
                    "post-S29 map dimensions not divisible by 48: "
                    f"{post29_map_width}x{post29_map_height}"
                )
            post29_map_cols = post29_map_width // 48
            post29_map_rows = post29_map_height // 48
            post29_terrain_cells = extract_hexzmap_cells(
                hexz,
                next_s_after29["number"],
                post29_map_cols,
                post29_map_rows,
            )
            (map_dir / next_s_after29_map_name).write_bytes(
                next_s_after29_map_bytes
            )
            (
                battle_dir
                / f"terrain{next_s_after29['number']}.bin"
            ).write_bytes(post29_terrain_cells)
            post29_map_probe.update({
                "valid": True,
                "width": post29_map_width,
                "height": post29_map_height,
                "cols": post29_map_cols,
                "rows": post29_map_rows,
                "terrainCellCount": len(post29_terrain_cells),
                "terrainIds": sorted(set(post29_terrain_cells)),
            })
        except Exception as exc:
            post29_map_probe.update({
                "valid": False,
    ...
    if (
        next_s_after30
        and next_s_after30_map_bytes is not None
    ):
        try:
            post30_map_width, post30_map_height = jpeg_dimensions(
                next_s_after30_map_bytes
            )
            if (
                post30_map_width % 48 != 0
                or post30_map_height % 48 != 0
            ):
                raise ValueError(
                    "post-S30 map dimensions not divisible by 48: "
                    f"{post30_map_width}x{post30_map_height}"
                )
            post30_map_cols = post30_map_width // 48
            post30_map_rows = post30_map_height // 48
            post30_terrain_cells = extract_hexzmap_cells(
                hexz,
                next_s_after30["number"],
                post30_map_cols,
                post30_map_rows,
            )
            (map_dir / next_s_after30_map_name).write_bytes(
                next_s_after30_map_bytes
            )
            (
                battle_dir
                / f"terrain{next_s_after30['number']}.bin"
            ).write_bytes(post30_terrain_cells)
            post30_map_probe.update({
                "valid": True,
                "width": post30_map_width,
                "height": post30_map_height,
                "cols": post30_map_cols,
                "rows": post30_map_rows,
                "terrainCellCount": len(post30_terrain_cells),
                "terrainIds": sorted(set(post30_terrain_cells)),
            })
        except Exception as exc:
            post30_map_probe.update({
                "valid": False,
    ...
    if next_s_after31 and next_s_after31_map_bytes is not None:
        try:
            post31_map_width, post31_map_height = jpeg_dimensions(
                next_s_after31_map_bytes
            )
            if post31_map_width % 48 != 0 or post31_map_height % 48 != 0:
                raise ValueError(
                    "post-S31 map dimensions not divisible by 48: "
                    f"{post31_map_width}x{post31_map_height}"
                )
            post31_map_cols = post31_map_width // 48
            post31_map_rows = post31_map_height // 48
            post31_terrain_cells = extract_hexzmap_cells(
                hexz,
                next_s_after31["number"],
                post31_map_cols,
                post31_map_rows,
            )
            (map_dir / next_s_after31_map_name).write_bytes(
                next_s_after31_map_bytes
            )
            (battle_dir / f"terrain{next_s_after31['number']}.bin").write_bytes(
                post31_terrain_cells
            )
            post31_map_probe.update({
                "valid": True,
                "width": post31_map_width,
                "height": post31_map_height,
                "cols": post31_map_cols,
                "rows": post31_map_rows,
                "terrainCellCount": len(post31_terrain_cells),
                "terrainIds": sorted(set(post31_terrain_cells)),
            })
        except Exception as exc:
            post31_map_probe.update({
                "valid": False,
    ...
    if s21 and s21.startswith(b"EEX"):
        s21_scenes = parse_scenario_tree(s21)
        s21_init_probe = probe_s01_initialization(s21)
    ...
    r22_probe = build_next_scenario_probe("R_22.eex", r22)
    s22_probe = build_next_scenario_probe("S_22.eex", s22)
    s22_init_probe = {
        "found": s22 is not None,
        "validEex": bool(s22 and s22.startswith(b"EEX")),
        "map": map22_probe,
    }
    s22_event_probe = []
    s22_outcome_probe = {}
    if s22 and s22.startswith(b"EEX"):
        s22_scenes = parse_scenario_tree(s22)
        s22_init_probe = probe_s01_initialization(s22)
    ...
    post_s25_probe = {
        ...
        "nextMap": post25_map_probe,
        "nextSInit": (
            probe_s01_initialization(next_s_after25_blob)
            if next_s_after25_blob
            else None
        ),
        "nextSEventSummary": None,
        "nextSOutcomeProbe": None,
    ...
    if (
        next_s_after25_blob
        and next_s_after25_blob.startswith(b"EEX")
    ):
        post25_scenes = parse_scenario_tree(
            next_s_after25_blob
        )
        post25_events = extract_scene2_native_events(
            post25_scenes
        )
        post_s25_probe["nextSEventSummary"] = {
            "candidateCount": len(post25_events),
            "coreSupportedCount": sum(
                1
                for event in post25_events
                if event["coreSupported"]
            ),
            "unsupportedSections": [
                {
        ...
        post_s25_probe["nextSOutcomeProbe"] = (
    ...
    r29 = next_r_after28_blob
    s29 = next_s_after28_blob
    r29_story = compile_r29_story(r29)
    r29_player_variants = extract_r29_departure_variants(r29)

    if (
        next_r_after28 is None
        or int(next_r_after28["number"]) != 29
        or next_s_after28 is None
        or int(next_s_after28["number"]) != 29
    ):
        raise SystemExit(
            "Expected R_29/S_29 after S28, got "
            + repr({
                "nextR": next_r_after28,
                "nextS": next_s_after28,
            })
        )
    if not post28_map_probe.get("valid"):
        raise SystemExit(
            "M029 map probe invalid: " + repr(post28_map_probe)
        )
    if not r29_story.get("supported"):
        raise SystemExit(
            "R29 story unsupported: "
            + repr(r29_story.get("unsupportedActionIds", []))
        )
    if len(r29_player_variants) != 2:
        raise SystemExit(
            "R29 expected two variable-1051 deployment variants, got "
            + repr(r29_player_variants)
        )

    s29_init_probe = probe_s01_initialization(s29)
    s29_init_probe["map"] = post28_map_probe
    s29_scenes = parse_scenario_tree(s29)
    s29_event_probe = extract_scene2_native_events(s29_scenes)
    s29_terminal_sections = set(range(50, 62)) | {70, 71, 72}
    s29_native_events = [
        event for event in s29_event_probe
        if event["section"] not in s29_terminal_sections
    ]
    s29_defeat_sections = {
        0: 50,
        2: 51,
    ...
    s29_outcome_events = {
        "defeatByCharacter": {
            str(cid): compile_scenario_section_actions(
                s29_scenes, 2, section
            )
            for cid, section in s29_defeat_sections.items()
        },
        "victory": compile_scenario_section_actions(
            s29_scenes, 2, 70
        ),
        "genericVictory": compile_scenario_section_actions(
            s29_scenes, 2, 71
        ),
        "genericDefeat": compile_scenario_section_actions(
            s29_scenes, 2, 72
    ...
    s29_outcome_probe = probe_battle_outcome_candidates(s29_scenes)

    s29_player_slot_groups = []
    current_slots = []
    for row in s29_init_probe.get("playerSlots", []):
        if int(row["slot"]) == 0 and current_slots:
            s29_player_slot_groups.append(current_slots)
            current_slots = []
        current_slots.append(row)
    if current_slots:
        s29_player_slot_groups.append(current_slots)

    if len(s29_player_slot_groups) != 2:
        raise SystemExit(
            "S29 expected two player-slot groups, got "
            + repr(s29_player_slot_groups)
        )

    s29_skipped_actors = []
    s29_nonplayer_units = []
    s29_character_ids = set()
    s29_reserved_player_ids = {
        int(cid)
        for variant in r29_player_variants
    ...
    s29_player_variants = []
    slot_groups_by_count = {
        len(group): group
        for group in s29_player_slot_groups
    }
    for variant_index, variant in enumerate(r29_player_variants):
        ids = variant["characterIds"]
        slots = slot_groups_by_count.get(len(ids))
        if slots is None:
            raise SystemExit(
                f"S29 no slot group for {len(ids)} players: "
                + repr(s29_player_slot_groups)
            )
        variant_units = []
        for slot, cid in zip(slots, ids):
            record = make_s29_record(
                cid,
                PLAYER,
                bool(slot.get("flag", 0)),
                slot["x"],
                ...
                0,
                False,
                f"S_29:0x4B:variant{variant_index}:{slot['slot']}",
                battle_number=int(slot["slot"]),
            )
            if record is None:
                raise SystemExit(f"S29 player {cid} invalid sprite")
            variant_units.append(record)
        s29_player_variants.append({
            "requireTrueVariables": variant["requireTrueVariables"],
            "requireFalseVariables": variant["requireFalseVariables"],
            "characterIds": ids,
            "units": variant_units,
        })

    next_battle_number = max(
        len(v["characterIds"]) for v in r29_player_variants
    )
    for index, row in enumerate(s29_init_probe.get("friendRecords", [])):
        cid = int(row["person"])
        if cid in s29_reserved_player_ids or cid in s29_character_ids:
            continue
        record = make_s29_record(
            cid,
            ALLY,
        ...
        if record is not None:
            s29_nonplayer_units.append(record)
            s29_character_ids.add(cid)
            next_battle_number += 1

    for index, row in enumerate(s29_init_probe.get("enemyRecords", [])):
        cid = int(row["person"])
        if cid in s29_reserved_player_ids or cid in s29_character_ids:
            continue
        record = make_s29_record(
    ...
    s29_objective_text = (
        s29_init_probe["objectiveTexts"][0]
        if s29_init_probe.get("objectiveTexts")
        else ""
    )
    s29_popup_text = (
        s29_init_probe["objectivePopups"][0]
        if s29_init_probe.get("objectivePopups")
        else ""
    )
    s29_turn_limit = objective_turn_limit(
        s29_objective_text,
        25,
    )
    s29_protected_ids = sorted(s29_reserved_player_ids)

    s29_battle = {
        "version": 118,
        "source": "RS/S_29.eex",
        "battleMode": "s29-eight-gates-breakthrough",
        "mapId": 29,
        "map": "m029.jpg",
        "widthTiles": post28_map_probe["cols"],
        "heightTiles": post28_map_probe["rows"],
        "terrainFile": "terrain29.bin",
        "terrainTypeCount": TERRAIN_TYPE_COUNT,
        "movementCostFile": "movement_costs.bin",
        "movementCostFamilyCount": JOB_FAMILY_COUNT,
        "terrainPowerFile": "terrain_power.bin",
        ...
        "battleObjectives": {
            "phase1": {
                "objectiveText": s29_objective_text,
                "popupText": s29_popup_text,
                "turnLimit": s29_turn_limit,
                "goal": {
                    "type": "event-driven-three-camps",
                    "targetCharacterId": 41,
                },
            },
            "phase2": {
                "objectiveText": s29_objective_text,
            ...
            "protectedCharacterIds": s29_protected_ids,
            "protectedCharacters": [
                {"characterId": cid, "name": name_of(cid)}
                for cid in s29_protected_ids
            ],
            "phase1TransitionEvents": [],
        },
        "battleEvents": s29_native_events,
        "outcomeEvents": s29_outcome_events,
        "outcomeProbe": s29_outcome_probe,
        "routeModel": {
            "victoryTargetCharacterId": 41,
            "victorySection": 70,
            "genericVictorySection": 71,
            "genericDefeatSection": 72,
            ...
            "postBattleScene": "S03-SEC01",
            "turnLimit": s29_turn_limit,
            "completionVariable": 629,
            "allPlayerDeathsDefeat": True,
        },
        "deploymentModel": {
            "rosterVariable": 1051,
            "variants": [
        ...
        "battleEventSummary": {
            "candidateCount": len(s29_native_events),
            "coreSupportedCount": sum(
                1 for event in s29_native_events
                if event["coreSupported"]
            ),
            "rawCandidateCount": len(s29_event_probe),
            "rawCoreSupportedCount": sum(
                1 for event in s29_event_probe
                if event["coreSupported"]
            ),
            "sections": [
                {
                ...
                for event in s29_native_events
            ],
        },
        "terrainIds": post28_map_probe["terrainIds"],
        "combatModel": COMBAT_MODEL,
        "damageModel": DAMAGE_MODEL,
        "supportedAttackRangeIds": [0, 1],
        "skippedActors": s29_skipped_actors,
        "playerVariants": s29_player_variants,
        "units": s29_nonplayer_units,
        "openingEvents": [],
    }

    s28_battle["r29Story"] = r29_story
    ...
    post_s29_probe = {
        ...
        "nextMap": post29_map_probe,
        "nextSInit": (
            probe_s01_initialization(next_s_after29_blob)
            if next_s_after29_blob
            else None
        ),
        "nextSEventSummary": None,
        "nextSOutcomeProbe": None,
        "nextRDepartureProbe": None,
        "s29SkippedActorReferences": [],
    }

    if s29_skipped_actors:
        flat29 = flatten_scenario_nodes(s29_scenes)
        actor_commands = {
            0x25, 0x26, 0x2E, 0x31, 0x32, 0x36,
            0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x53,
            0x55, 0x5C, 0x68, 0x75, 0x78,
        }
        for actor in s29_skipped_actors:
            cid = int(actor["characterId"])
            refs = []
            for row in flat29:
                if row["commandId"] not in actor_commands:
                    continue
            ...
            post_s29_probe["s29SkippedActorReferences"].append({
                **actor,
    ...
    if (
        next_s_after29_blob
        and next_s_after29_blob.startswith(b"EEX")
    ):
        post29_scenes = parse_scenario_tree(next_s_after29_blob)
        post29_events = extract_scene2_native_events(post29_scenes)
        post_s29_probe["nextSEventSummary"] = {
            "candidateCount": len(post29_events),
            "coreSupportedCount": sum(
                1 for event in post29_events
                if event["coreSupported"]
            ),
            "unsupportedSections": [
                {
                ...
                for event in post29_events
                if not event["coreSupported"]
            ],
        }
        post_s29_probe["nextSOutcomeProbe"] = (
            probe_battle_outcome_candidates(post29_scenes)
        )

    if (
        next_r_after29_blob
        and next_r_after29_blob.startswith(b"EEX")
    ):
        post29_r_scenes = parse_scenario_tree(next_r_after29_blob)
        if post29_r_scenes:
            departure_scene_number = len(post29_r_scenes)
            departure_flat = flatten_scenario_nodes(
                [post29_r_scenes[-1]]
            )
            post_s29_probe["nextRDepartureProbe"] = {
                "scene": departure_scene_number,
                "sectionCount": len(
                    post29_r_scenes[-1]["sections"]
                ),
                "commands": [
                    {
                        "section": row["section"],
    ...
    post_s30_probe = {
        "inventoryAfter30": [
            row for row in rs_inventory if row["number"] > 30
        ],
        "nextR": (
            {
                **next_r_after30,
        ...
        "nextMap": post30_map_probe,
        "nextSInit": (
            probe_s01_initialization(next_s_after30_blob)
            if next_s_after30_blob
            else None
        ),
        "nextSEventSummary": None,
        "nextSOutcomeProbe": None,
    ...
    if (
        next_s_after30_blob
        and next_s_after30_blob.startswith(b"EEX")
    ):
        post30_scenes = parse_scenario_tree(next_s_after30_blob)
        post30_events = extract_scene2_native_events(post30_scenes)
        post_s30_probe["nextSEventSummary"] = {
            "candidateCount": len(post30_events),
            "coreSupportedCount": sum(
                1 for event in post30_events
                if event["coreSupported"]
            ),
            "unsupportedSections": [
                {
                ...
                for event in post30_events
                if not event["coreSupported"]
            ],
        }
        post_s30_probe["nextSOutcomeProbe"] = (
            probe_battle_outcome_candidates(post30_scenes)
        )

    if (
        next_r_after30_blob
        and next_r_after30_blob.startswith(b"EEX")
    ):
        post30_r_scenes = parse_scenario_tree(next_r_after30_blob)
        if post30_r_scenes:
            departure_scene_number = len(post30_r_scenes)
            departure_flat = flatten_scenario_nodes(
                [post30_r_scenes[-1]]
            )
            post_s30_probe["nextRDepartureProbe"] = {
                "scene": departure_scene_number,
                "sectionCount": len(
                    post30_r_scenes[-1]["sections"]
                ),
                "commands": [
                    {
                        "section": row["section"],
    ...
    s30_battle["postS30Probe"] = post_s30_probe
    (battle_dir / "battle30.json").write_text(
        json.dumps(s30_battle, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    post_s31_probe = {
        "inventoryAfter31": [
            row for row in rs_inventory if row["number"] > 31
        ],
        "nextR": (
            {
                **next_r_after31,
                "probe": build_next_scenario_probe(
                    next_r_after31["filename"],
                    next_r_after31_blob,
                ),
            }
            if next_r_after31
        ...
        "nextMap": post31_map_probe,
        "nextSInit": (
            probe_s01_initialization(next_s_after31_blob)
            if next_s_after31_blob
            else None
        ),
        "nextSEventSummary": None,
        "nextSOutcomeProbe": None,
    ...
    if next_s_after31_blob and next_s_after31_blob.startswith(b"EEX"):
        post31_scenes = parse_scenario_tree(next_s_after31_blob)
        post31_events = extract_scene2_native_events(post31_scenes)
        post_s31_probe["nextSEventSummary"] = {
            "candidateCount": len(post31_events),
            "coreSupportedCount": sum(
                1 for event in post31_events if event["coreSupported"]
            ),
            "unsupportedSections": [
                {
                    "section": event["section"],
                ...
                for event in post31_events
                if not event["coreSupported"]
            ],
        }
        post_s31_probe["nextSOutcomeProbe"] = (
            probe_battle_outcome_candidates(post31_scenes)
        )

    if next_r_after31_blob and next_r_after31_blob.startswith(b"EEX"):
        post31_r_scenes = parse_scenario_tree(next_r_after31_blob)
        if post31_r_scenes:
            departure_scene_number = len(post31_r_scenes)
            departure_flat = flatten_scenario_nodes([post31_r_scenes[-1]])
            post_s31_probe["nextRDepartureProbe"] = {
                "scene": departure_scene_number,
                "sectionCount": len(post31_r_scenes[-1]["sections"]),
                "commands": [
                    {
                        "section": row["section"],
                        "depth": row["depth"],
    ...
    s31_battle["postS31Probe"] = post_s31_probe
    ...
    r32 = next_r_after31_blob
    s32 = next_s_after31_blob
    r32_story = compile_r32_story(r32)

    if (
        next_r_after31 is None
        or int(next_r_after31["number"]) != 32
        or next_s_after31 is None
        or int(next_s_after31["number"]) != 32
    ):
        raise SystemExit(
            "Expected R_32/S_32 after S31, got "
            + repr({
                "nextR": next_r_after31,
                "nextS": next_s_after31,
            })
        )
    if not post31_map_probe.get("valid"):
        raise SystemExit(
            "M032 map probe invalid: " + repr(post31_map_probe)
        )
    if not r32_story.get("supported"):
        raise SystemExit(
            "R32 story unsupported: "
            + repr(r32_story.get("unsupportedActionIds", []))
        )

    s32_init_probe = probe_s01_initialization(s32)
    s32_init_probe["map"] = post31_map_probe
    s32_scenes = parse_scenario_tree(s32)
    s32_event_probe = extract_scene2_native_events(s32_scenes)

    s32_victory_candidates = []
    for event in s32_event_probe:
        if int(event.get("section", -1)) in {55, 56, 57}:
            continue
        has_end = action_tree_contains_type(
            event.get("actions", []),
            {"battleEndMarker"},
        )
        has_failure = action_tree_contains_type(
            event.get("actions", []),
            {"battleFailureMarker"},
        )
        if has_end and not has_failure:
            s32_victory_candidates.append({
                "section": int(event["section"]),
    ...
    s32_native_events = [
        ...
        if int(event["section"]) not in s32_terminal_sections
    ]
    s32_outcome_events = {
        "defeatByCharacter": {
            "4": compile_scenario_section_actions(
    ...
    s32_outcome_probe = probe_battle_outcome_candidates(s32_scenes)

    s32_slots = sorted(
        s32_init_probe.get("playerSlots", []),
        key=lambda row: row["slot"],
    )
    s32_player_ids = extract_forced_player_roster(
        s32,
        len(s32_slots),
    )
    if len(s32_player_ids) != len(s32_slots):
        raise SystemExit(
            "S32 forced roster/slot mismatch: "
            + repr({
                "players": s32_player_ids,
                "slots": s32_slots,
                "forcedRaw": s32_init_probe.get("forcedPlayers", []),
            })
        )

    s32_units = []
    s32_skipped_actors = []
    s32_character_ids = set()
    ...
    for slot, cid in zip(s32_slots, s32_player_ids):
        record = make_s32_record(
            cid,
            PLAYER,
            bool(slot.get("flag", 0)),
            slot["x"],
            ...
            0,
            False,
            f"S_32:forced:slot:{slot['slot']}",
            battle_number=int(slot["slot"]),
        )
        if record is None:
            raise SystemExit(f"S32 player {cid} invalid sprite")
        s32_units.append(record)
        s32_character_ids.add(int(cid))

    next_battle_number = len(s32_units)
    for index, row in enumerate(
        s32_init_probe.get("friendRecords", [])
    ):
        cid = int(row["person"])
        if cid in s32_character_ids:
            continue
        record = make_s32_record(
            cid,
            ALLY,
        ...
        if record is not None:
            s32_units.append(record)
            s32_character_ids.add(cid)
            next_battle_number += 1

    for index, row in enumerate(
        s32_init_probe.get("enemyRecords", [])
    ):
        cid = int(row["person"])
        if cid in s32_character_ids:
            continue
    ...
    s32_objective_text = (
        s32_init_probe["objectiveTexts"][0]
        if s32_init_probe.get("objectiveTexts")
        else ""
    )
    s32_popup_text = (
        s32_init_probe["objectivePopups"][0]
        if s32_init_probe.get("objectivePopups")
        else ""
    )
    s32_turn_limit = objective_turn_limit(
        s32_objective_text,
        70,
    )
    ...
    s32_battle = {
        ...
        "mapId": 32,
        "map": "m032.jpg",
        "widthTiles": post31_map_probe["cols"],
        "heightTiles": post31_map_probe["rows"],
        "terrainFile": "terrain32.bin",
        "terrainTypeCount": TERRAIN_TYPE_COUNT,
        "movementCostFile": "movement_costs.bin",
        "movementCostFamilyCount": JOB_FAMILY_COUNT,
        "terrainPowerFile": "terrain_power.bin",
        ...
        "battleObjectives": {
            "phase1": {
                "objectiveText": s32_objective_text,
                "popupText": s32_popup_text,
                "turnLimit": s32_turn_limit,
                "goal": {
                    "type": "find-ladies",
                    "targetCharacterId": 4,
                },
            ...
            "protectedCharacterIds": [4, 26],
            "protectedCharacters": [
                {"characterId": 4, "name": name_of(4)},
                {"characterId": 26, "name": name_of(26)},
            ],
            "phase1TransitionEvents": [],
        },
        "battleEvents": s32_native_events,
        "outcomeEvents": s32_outcome_events,
        ...
        "battleEventSummary": {
            "candidateCount": len(s32_native_events),
            "coreSupportedCount": sum(
                1 for event in s32_native_events
                if event["coreSupported"]
            ),
            "rawCandidateCount": len(s32_event_probe),
            "rawCoreSupportedCount": sum(
                1 for event in s32_event_probe
                if event["coreSupported"]
            ),
            "sections": [
                {
                ...
                for event in s32_native_events
            ],
        },
        "terrainIds": post31_map_probe["terrainIds"],
        "combatModel": COMBAT_MODEL,
        "damageModel": DAMAGE_MODEL,
        "supportedAttackRangeIds": [0, 1],
        "skippedActors": s32_skipped_actors,
        "playerRoster": {
            "characterIds": s32_player_ids,
            "source": "S_32 Scene1 forced 0x4A roster",
        },
        "units": s32_units,
        "openingEvents": [],
        "r32Story": r32_story,
    ...
    s18_battle = {
        ...
        "postS18Probe": {
            ...
            "s19EventSummary": {
                "candidateCount": len(s19_event_probe),
                "coreSupportedCount": sum(
                ...
                "unsupportedSections": [
                    ...
                    for event in s19_event_probe
                    if not event["coreSupported"]
                ],
            },
            "s19OutcomeSections": sorted(s19_outcome_probe.keys()),
            "s19OutcomeProbe": s19_outcome_probe,
        },
        "routeModel": {
    ...
    s20_battle = {
        ...
        "postS20Probe": {
            ...
            "s21EventSummary": {
                "candidateCount": len(s21_event_probe),
                "coreSupportedCount": sum(
                ...
                "unsupportedSections": [
                    ...
                    for event in s21_event_probe
                    if not event["coreSupported"]
                ],
            },
            "s21OutcomeSections": sorted(s21_outcome_probe.keys()),
            "s21OutcomeProbe": s21_outcome_probe,
        },
        "routeModel": {
    ...
    s21_battle = {
        ...
        "postS21Probe": {
            ...
            "s22EventSummary": {
                "candidateCount": len(s22_event_probe),
                "coreSupportedCount": sum(
            ...
            "s22OutcomeSections": sorted(
                int(key)
                for key in s22_outcome_probe.keys()
                if str(key).isdigit()
            ) if isinstance(s22_outcome_probe, dict) else [],
            "s22OutcomeProbe": s22_outcome_probe,
        },
        "routeModel": {
    ...
    if s22 and s22.startswith(b"EEX"):
        for row in flatten_scenario_nodes(s22_scenes):
            if row.get("scene") != 2 or row.get("commandId") != 0x36:
                continue
            params = row.get("params", [])
    ...
    s23_outcome_events = {
        "victory": compile_scenario_section_actions(s23_scenes, 2, 33),
        "genericVictory": compile_scenario_section_actions(s23_scenes, 2, 40),
        "defeatByCharacter": {
            "0": compile_scenario_section_actions(s23_scenes, 2, 35),
            "25": compile_scenario_section_actions(s23_scenes, 2, 36),
            "4": compile_scenario_section_actions(s23_scenes, 2, 37),
        },
        "genericDefeat": compile_scenario_section_actions(s23_scenes, 2, 41),
        "postBattle": compile_scenario_section_actions(s23_scenes, 3, 1),
    }
    ...
    for slot, cid in zip(s23_slots, s23_player_ids):
        if not make_s23_unit(
            ...
            raise SystemExit(
                f"S23 player {cid} has invalid default sprite"
            )
    ...
    s23_objective_text = (
        s23_objective_texts[0] if s23_objective_texts else ""
    )
    s23_popup_text = (
        s23_popup_texts[0] if s23_popup_texts else ""
    )
    ...
    s23_battle = {
        ...
        "battleObjectives": {
            "phase1": {
                ...
                "goal": {
                    "type": "kill-character-or-annihilate",
                    "characterId": 36,
                    "name": name_of(36),
                },
            },
    ...
    # R24 -> S24 playable escort battle.
    ...
    if s24_player_ids != [1, 29]:
        raise SystemExit(
            "S24 forced roster unexpected: " + repr(s24_player_ids)
        )
    ...
    if len(s24_slots) != len(s24_player_ids):
        raise SystemExit(
            "S24 roster/slot mismatch: "
            + repr({
    ...
    for slot, cid in zip(s24_slots, s24_player_ids):
        # 0x4C mode 1 addresses runtime battle numbers. In S24,
        # battle number 1 is the second player slot (Sun Qian), as
        # confirmed by Sections 22/23 immediately acting/dialoguing as 29.
    ...
    s24_objective_text = (
        s24_init_probe["objectiveTexts"][0]
        if s24_init_probe.get("objectiveTexts")
        else ""
    )
    s24_popup_text = (
        s24_init_probe["objectivePopups"][0]
        if s24_init_probe.get("objectivePopups")
        else ""
    )
    s24_turn_limit = objective_turn_limit(s24_objective_text, 10)
    s24_route_objective = (
        "승리 조건\n★·관우와 마차가 하북에 도착.\n\n"
        "패배 조건\n☆·관우 사망.\n☆·마차 격퇴.\n☆·80턴 초과."
    )
    ...
    s24_battle = {
        "version": 99,
        "source": "RS/S_24.eex",
        "battleMode": "s24-carriage-escort",
        "mapId": 24,
        "map": "m024.jpg",
        ...
        "battleObjectives": {
            ...
            "phase2": {
                "objectiveText": s24_route_objective,
                "popupText": "하북에 도착하라",
                "turnLimit": 80,
                "goal": {
                    "type": "escort-area",
            ...
            "protectedCharacters": [
                {"characterId": 1, "name": name_of(1)},
                {"characterId": 327, "name": name_of(327)},
            ],
            "phase1TransitionEvents": [],
        },
        ...
        "r25Story": r25_story,
        "s25InitProbe": s25_init_probe,
        "routeModel": {
            "carriageFoundSection": 3,
            "routeSections": [22, 23],
            "routeVariables": [17, 18],
            "acceptEscortVariable": 150,
            "manualCarriageDirectionVariable": 160,
            "victoryArea": {
                "x1": 0,
                "y1": 0,
                "x2": 22,
    ...
    # R25 -> S25: original nine-slot continuation battle.
    ...
    expected_s25_players = [0, 1, 2, 4, 17, 25, 27, 190, 191]
    if s25_player_ids != expected_s25_players:
        raise SystemExit(
            "R25 departure roster unexpected: "
            + repr({
                "actual": s25_player_ids,
                "expected": expected_s25_players,
                "selectable": r25_selectable_ids,
            })
        )
    ...
    if len(s25_slots) != len(s25_player_ids):
        raise SystemExit(
    ...
    for slot, cid in zip(s25_slots, s25_player_ids):
        if not make_s25_unit(
            ...
            raise SystemExit(
                f"S25 player {cid} duplicate or invalid sprite"
            )

    next_battle_number = len(s25_slots)
    for index, row in enumerate(
    ...
    s25_battle = {
        ...
        "battleObjectives": {
            ...
            "protectedCharacterIds": [0, 337],
            "protectedCharacters": [
                {"characterId": 0, "name": name_of(0)},
                {"characterId": 337, "name": name_of(337)},
            ],
            "phase1TransitionEvents": [],
        },
