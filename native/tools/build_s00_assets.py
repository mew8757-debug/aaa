...
def main(argv):
    ...
    with zipfile.ZipFile(game1_path) as game1, zipfile.ZipFile(game2_path) as game2:
        ...
        if (
            next_s_after31_map_bytes is None
            and next_s_after31_map_name
        ):
    ...
    if (
        next_s_after31
        and next_s_after31_map_bytes is not None
    ):
        try:
            ...
            if (
                post31_map_width % 48 != 0
                or post31_map_height % 48 != 0
            ):
                raise ValueError(
            ...
            (
                battle_dir
                / f"terrain{next_s_after31['number']}.bin"
            ).write_bytes(post31_terrain_cells)
    ...
    if (
        next_s_after31_blob
        and next_s_after31_blob.startswith(b"EEX")
    ):
    ...
    if (
        next_r_after31_blob
        and next_r_after31_blob.startswith(b"EEX")
    ):
